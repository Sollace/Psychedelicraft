    ...
    public static void renderLightsScreen(MatrixStack matrices, VertexConsumer vertices, float u, float v, float ticks, float alpha, int color, int number) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (lightAlpha > 0.01F) {
                ...
                float opaque = alpha * lightAlpha;
                float transparent = 0;

                vertices.vertex(pos.x, pos.y, pos.z, 1, 1, 1, opaque, 0, 0, light, 0, 1, 1, 1);

                vector.set(-width * var9, var8, -0.5F * var9, 1);
                pos = positionMatrix.transform(vector);
                vertices.vertex(pos.x, pos.y, pos.z, 1, 1, 1, transparent, 1, 0, light, 0, 1, 1, 1);

                vector.set(width * var9, var8, -0.5F * var9, 1);
                pos = positionMatrix.transform(vector);
                vertices.vertex(pos.x, pos.y, pos.z, 1, 1, 1, transparent, 0, 1, light, 0, 1, 1, 1);
