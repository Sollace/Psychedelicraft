...
public class SmeltingFluidEmiRecipe implements EmiRecipe {
    ...
    public SmeltingFluidEmiRecipe(Identifier id, SmeltingFluidRecipe recipe) {
        ...
        output = input.getEmiStacks().stream().map(stack -> recipe.modifyStack(stack.getItemStack())).map(EmiStack::of).toList();
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        widgets.addFillingArrow(24, 5, 50 * recipe.getCookTime())
            .tooltipText(List.of(Text.translatable("emi.cooking.time", recipe.getCookTime() / 20f)));

        widgets.addTexture(EmiTexture.EMPTY_FLAME, 1, 24);
        widgets.addAnimatedTexture(EmiTexture.FULL_FLAME, 1, 24, 4000 / recipe.getCookTime(), false, true, true);
