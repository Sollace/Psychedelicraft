...
class DryingEmiRecipe implements PSRecipe {
    ...
    public DryingEmiRecipe(DryingRecipe recipe) {
        this.recipe = recipe;
        TlaIngredient input = TlaIngredient.ofIngredient(recipe.input());
    ...
    public void buildGui(GuiBuilder widgets) {
        ...
        widgets.addText(Text.translatable("psychedelicraft.recipe.experience", recipe.experience()), 58, 55, Colors.WHITE, true);

        widgets.addTexture(SUN, 95, 0);

        long cookingTime = DryingTableBlockEntity.getCookingTime(recipe.cookTime(), false);
