...
class DrawingFluidEmiRecipe implements PSRecipe {
    ...
    public DrawingFluidEmiRecipe(RecipeCategory category, RecipeUtil.Contents contents, int capacity) {
        ...
        this.id = category.getId().withSuffixedPath("/" + contents.type().fluid().getId().toUnderscoreSeparatedString() + contents.type().fluid().getUniqueKey(contents.type())).withPrefixedPath("/");
