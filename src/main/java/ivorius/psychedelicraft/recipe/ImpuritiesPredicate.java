...
public record ImpuritiesPredicate(Set<Impurity> require, Set<Impurity> reject, Optional<Set<Impurity>> allow) implements Predicate<Impurities> {
    ...
    private static final PacketCodec<PacketByteBuf, Set<Impurity>> IMPURITY_SET_PACKET_CODEC = PacketCodecUtils.ofEnum(Impurity.class).collect(PacketCodecs.toCollection(HashSet::new));
    public static final PacketCodec<PacketByteBuf, ImpuritiesPredicate> PACKET_CODEC = PacketCodec.tuple(
